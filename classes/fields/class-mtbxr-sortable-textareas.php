...
if(!class_exists('Mtbxr_Sortable_Textareas')):
	
	if(MTBXR_DEBUG){
		$mtbxr_debug_info[] = 'Included class Mtbxr_Sortable_Textareas in '.__FILE__;
	}
	
	class Mtbxr_Sortable_Textareas extends Mtbxr_Field{
		...
		public function render_main_view($meta, $post_id, $field_class=''){
			...
			$vars['textareas'] = array();
			foreach($vars['saved_values'] as $value){
				$vars['textareas'][] = $value;
			}
